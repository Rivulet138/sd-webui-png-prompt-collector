...
from .service import build_prompt_batch, collect_positive_prompts, export_prompt_batch, import_prompt_batch
...
def _collect(uploaded, directory, recursive, deduplicate):
    result = collect_positive_prompts(uploaded, directory, recursive)
    try:
        batch = build_prompt_batch(result.records, deduplicate)
    ...
    if not batch["records"]:
        return _status("error", "未读取到正向 Prompt", f"检查了 {result.selected_count} 张 PNG"), [], batch, None, error_text
    kind = "warning" if result.skipped_count else "success"
    detail = f"读取 {result.imported_count}/{result.selected_count} 张；当前批次 {len(batch['records'])} 条"
    return _status(kind, "逐图 Prompt 已读取", detail), _rows(batch), batch, export_prompt_batch(batch) if batch["records"] else None, error_text
